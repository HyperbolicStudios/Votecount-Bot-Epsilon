...
class RIVA(app_commands.Group):
    ...
    async def add(self, interaction: discord.Interaction, game: Literal['A', 'B', 'C'], player: str):
    ...
    async def sub(self, interaction: discord.Interaction, game: Literal['A', 'B', 'C'], player: str):
        increments = getData("increments"+game)
        #of form {'player':'increment'}
        increments.update({player.lower():increments[player.lower()]-0.2})
        updateData("increments"+game,increments)
    ...
    async def reset_counter(self, interaction: discord.Interaction, game: Literal['A', 'B', 'C']):
    ...
    @app_commands.check(is_host)
    @app_commands.describe(game='Available Games')
    async def print_counter(self, interaction: discord.Interaction, game: Literal['A', 'B', 'C']):
...
class Alias(app_commands.Group):
    ...
    async def add(self, interaction: discord.Interaction, alias: str, true_name: str):
    ...
    async def change_forum_name(self, interaction: discord.Interaction, old_name: str, new_name: str):
        list_of_aliases = getData("list_of_aliases")
